...
| **缓存淘汰** | LRU + TTL 双重淘汰，最大 500 条目 / 500MB，180 秒自动过期 |
| **即时 flush** | 每次 write 后立即 `wfile.flush()`，数据不积压在缓冲区 |
| **缓存命中流式发送** | 缓存命中也分块 64KB + flush 发送，不等整个文件写完 |
...
| `IPTV_MAX_CONTENT_LENGTH` | 52428800 | 代理最大内容长度(字节) || `IPTV_PRELOAD_MAX_ENTRIES` | 500 | 预加载缓存最大条目数 |
| `IPTV_PRELOAD_MAX_SIZE` | 524288000 | 预加载缓存最大总字节数(500MB) |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `IPTV_PRELOAD_MAX_ENTRIES` | 500 | 缓存最大条目数 |
| `IPTV_PRELOAD_MAX_SIZE` | 500MB | 缓存最大总内存 |
| `IPTV_PRELOAD_TTL` | 180s | 缓存过期时间 |
| `IPTV_PRELOAD_WORKERS` | 6 | 并发预拉取线程数 |
...
| 模式 | `PRELOAD_SYNC_ALL` | 行为 | m3u8 返回延迟 | 播放体验 |
|------|-------------------|------|---------------|---------|
| **默认（前3个同步）** | `false` | 前3个同步等，后续异步 | ~1-3s | 前9秒不卡，后续可能微卡 |
| **全量预热** | `true` | **所有 .ts 分片全部拉完** | ~5-15s | **全程零延迟，完全不卡** |

**全量预热模式**（推荐开启）：

```bash
export IPTV_PRELOAD_SYNC_ALL=true
python server.py
```

返回 m3u8 时，**所有有效 .ts 分片 URL 都已预加载到内存缓存**。浏览器请求任何分片都是缓存命中（零网络延迟），播放全程丝滑。

```
全量预热时间线：
  m3u8 → [并行拉.ts1,.ts2,...,tsN 全部完成] → 返回m3u8 → 浏览器请求任意.ts → 缓存命中(0ms) ✅
           6线程并发，通常5-15秒完成全部预热
```
...
浏览器收到 m3u8 后会立即请求前几个 `.ts` 分片。如果预加载是纯异步的，此时前几个分片还没拉完，浏览器只能走正常代理路径等待下载。

解决方案：对前 `PRELOAD_SYNC_FIRST` 个分片使用 `future.result()` **同步等待**预加载完成，确保返回 m3u8 时前 3 个分片已经在缓存中。浏览器请求时直接命中，零延迟。

```
之前：  m3u8 → [异步提交预加载] → 立刻返回m3u8 → 浏览器请求.ts1 → 还在拉...卡顿
现在：  m3u8 → [同步等待前3个.ts拉完] → 返回m3u8 → 浏览器请求.ts1 → 缓存命中(0ms) ✅
              后续.ts继续后台异步拉取
```
...
- ✅ **同步等待前 N 个分片**（`IPTV_PRELOAD_SYNC_FIRST=3`）：返回 m3u8 前同步等待前 3 个 .ts 分片预加载完成，消除前 15 秒卡顿
- ✅ **即时 flush**：流式转发和缓存命中发送时每次 write 后立即 flush，数据不积压在 Python 缓冲区
...
## 🐛 关键Bug修复与优化 (2026-07-01 最终版)

### ✅ 已解决的所有问题

#### 1️⃣ **bat脚本无法启动Web服务器（严重）**
**问题**: 运行bat脚本后，采集完成但8000端口Web服务器无法启动
**原因**: 
- 脚本在调用notify.py后直接 exit /b 0 提前退出
- notify.py进入无限循环while True阻塞了后续代码执行

**解决方案**:
`atch
# ❌ 修改前：提前退出
call :show_step_time "Total" "%SCRIPT_START_TIME%"
exit /b 0    # ← 这里导致脚本结束，Web服务器永远不会启动
...
# ✅ 修改后：移除exit，继续执行
call :show_step_time "Total" "%SCRIPT_START_TIME%"
# 移除了 exit /b 0，让脚本自然流向下一行

:setup_scheduled_task_and_web  # ← 现在可以正常执行到！
echo Starting local web server...
%PYTHON_CMD% server.py %SERVER_PORT%
`

#### 2️⃣ **bat文件BOM编码问题（严重）**
**现象**: 双击运行bat报错 '锘緻echo' 不是内部或外部命令
**原因**: 文件被保存为UTF-8 with BOM格式，Windows cmd.exe无法识别
**修复**: 转换为GBK/ANSI编码（无BOM）
...
---
 ...
 = [System.IO.File]::ReadAllText("iptv_tool.bat", [System.Text.UTF8Encoding]::new(False))
[System.IO.File]::WriteAllText("iptv_tool.bat", ## 📡Collect-IPTV
初始版本基于 DeepSeek 与 ChatGPT 生成，最新版本使用 Gemini 与 GPT-5.3-Codex 持续优化；依托 GitHub 服务器进行源地址可用性与延迟测试，网页已更新台标展示，并支持去重与优选低延迟最佳 URL，M3U 播放列表每 4 小时自动更新。
...
#### 3️⃣ **notify.py无限循环阻塞问题（关键）**
**问题**: 从bat/sh调用notify.py时，脚本卡住无法继续
**原因**: main()函数有while True持续监控循环
**解决方案**: 添加--once单次模式参数
...
`ash
# Linux/macOS sh
 "/script/notify.py" --once
`

#### 4️⃣ **访问根路径返回404错误**
**现象**: http://127.0.0.1:8000 返回404 File not found
**原因**: output/目录不存在，server.py从该目录提供静态文件服务
**修复**: 创建output目录并复制index.html

`ash
mkdir -p output
cp .github/workflows/index.html output/
`
...
`
[1/5] 检测Python环境          ✅ (0.x秒)
[2/5] 测试PIP镜像源           ✅ (3-4秒) → 选择最快镜像
[3/5] 检测虚拟环境            ✅ (找到 .venv)
[4/5] 安装依赖                ✅ (3秒) → 使用缓存
[5/5] 运行IPTV采集            ✅ (1-2秒) → 生成M3U/M3U8文件
     ↓
[*] 发送邮件通知（单次模式）   ✅ (首次运行或变更时发送)
     ↓
[*] 注册定时任务              ✅ (每4小时自动采集)
     ↓
========================================
Starting Local Web Server      ✅ 启动成功！
========================================
  访问地址: http://127.0.0.1:8000
  局域网地址: http://192.168.x.x:8000
  Serving: D:\ws\Collect-IPTV\output
  音频转码: 已启用 (FFmpeg: ffmpeg\windows\bin\ffmpeg.exe)
  
Press Ctrl+C to stop           ← 服务器持续运行
`
...
1. **编码规范**: Windows bat文件必须使用GBK/ANSI编码（无BOM），不能用UTF-8 BOM
2. **流程控制**: bat脚本不能随意使用exit /b 0，要确保所有必要步骤都执行完毕
3. **子进程管理**: Python脚本被其他脚本调用时要支持单次运行模式，避免无限循环
4. **静态文件服务**: Web服务器的文档根目录必须存在且包含index.html
5. **跨平台一致性**: Windows/Linux/macOS的功能和体验要保持统一
