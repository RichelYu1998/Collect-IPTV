...
**状态**: ✅ 已通过测试，可投入使用
 ...
 = [System.IO.File]::ReadAllText(, [System.Text.UTF8Encoding]::new(False))
[System.IO.File]::WriteAllText(, # 项目代码规范与范式 (Skill)
...
# 936 = GBK codepage for Chinese Windows
`

**预防措施**:
1. 编辑器设置: VSCode → 右下角编码 → 选择 "GBK"
2. Git配置: .gitattributes 中添加 *.bat text eol=crlf encoding=GBK
3. 自动化检查: CI流程中添加BOM检测步骤

---

#### 🟡 P2: 404 Not Found
**现象**: 访问 http://127.0.0.1:8000 返回404
**日志**:
`
127.0.0.1 - - [01/Jul/2026 11:38:48] "GET / HTTP/1.1" 404 -
`

**原因分析**:
`python
# server.py 第1507行
serve_dir = str((PROJECT_ROOT / "output").resolve())  # 文档根目录
handler = functools.partial(CORSProxyHandler, directory=serve_dir)

# 如果 output/ 目录不存在 → 请求根路径 "/" 时找不到 index.html → 404
`
...
`ash
# 创建output目录
mkdir -p D:\ws\Collect-IPTV\output

# 复制index.html（从GitHub Actions工作流目录）
cp .github/workflows/index.html output/

# 验证
ls -la output/
# 应该看到: index.html (53KB)
`
...
- [x] **工作目录验证**
  `atch
  echo %CD%
  预期: D:\ws\Collect-IPTV (非 script\)
  `

- [x] **FFmpeg路径检测**
  `
  [*] 使用预编译 FFmpeg: D:\ws\Collect-IPTV\ffmpeg\windows\bin
  [*] 找到预编译 FFmpeg: D:\ws\Collect-IPTV\ffmpeg\windows\bin\ffmpeg.exe
  `

- [x] **IPTV采集成功**
  `
  Valid streams: 1944, deduplicated: 1252, best-per-channel: 770
  Generated sorted M3U file: file/best_sorted.m3u
  Stream collection took: 1.0s
  `

- [x] **邮件通知单次模式**
  `
  [通知] 检测文件变更并发送邮件通知（单次模式）...
  [通知] 单次检测完成（无变更或发送失败）
  `

- [x] **定时任务注册**
  `
  [*] Scheduled task already exists: IPTV_Collection
  Status: Ready
  `
...
`
ed0d157 fix: 修复bat脚本完整流程 - 编码/通知模式/Web服务器启动
4471929 fix: 修复bat脚本无法启动8000端口Web服务器的严重问题
e3c357a feat: 优化邮件通知策略 - 首次运行或变化即发送
aedddff feat: 完善Windows bat脚本邮件通知和双击运行修复
d87e373 feat: notify.py 支持将 m3u/m3u8 文件作为邮件附件发送 📎✨
`
...
1. **Windows批处理脚本的坑**
   - 编码必须是GBK/ANSI，不能用UTF-8 BOM
   - exit /b 0要慎用，确保所有必要步骤已完成
   - 路径中的中文需要chcp 65001支持
...
3. **Web服务器的文档根目录**
   - 启动前检查目录是否存在
   - 提供有意义的默认页面（index.html）
   - 404时要给出清晰的错误信息

4. **跨平台开发的注意事项**
   - Windows/Linux/macOS的路径分隔符不同
   - 文件编码要考虑目标平台
   - 测试要在所有目标平台上进行
...
**最后更新**: 2026-07-01  
**版本**: v1.0.0 (所有Critical问题已修复，功能完整可用)  
**状态**: ✅ 生产就绪，通过完整测试
